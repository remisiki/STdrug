...
loadDrugRef <- function(
  ...
  # Load L1000
  l1000_phase1_path <- file.path(drug_ref_dir, "l1000", "GSE92742")
  l1000_phase2_path <- file.path(drug_ref_dir, "l1000", "GSE70138")
  fda_drugs <- readLines(file.path(drug_ref_dir, "fda.txt"))
  l1000 <- loadLincsTwoPhasesDrugResponse(l1000_phase1_path, l1000_phase2_path, fda_drugs, tissue)
  # Load Tahoe
  tahoe_path <- file.path(drug_ref_dir, "tahoe", "drug_ref.rds")
  ...
  # Combine perturbation datasets
  perturbation <- list(
    response = cbind(l1000$response, tahoe_response),
    sig_info = rbind(l1000$sig_info, tahoe_sig_info),
    gene_info = gene_info
  )
  # Load gpt-4o annotation
  annotation <- read.csv(file.path(drug_ref_dir, "drug_validation", paste0(tissue, ".csv")), row.names = 1)
  gdsc <- read.csv(file.path(drug_ref_dir, "gdsc.csv"), row.names = 1)
  sider <- read.csv(file.path(drug_ref_dir, "sider.csv"), row.names = 1)
